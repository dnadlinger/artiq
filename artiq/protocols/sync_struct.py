...
class ModAction(Enum):
    ...
    type of modification to make. Except for initial initialisation, the member
    object to apply it to is given in ``m["path"]`` as an array (givng successive
    levels of indices). Each action has its specific additional data fields.

    For example to append the value 42 to the array ``data.counts[0]``: ::
...
class Notifier:
    ...
    def append(self, x):
        ...
        if self.root.publish is not None:
            self.root.publish({"action": ModAction.append,
                               "path": self._path,
                               "x": x})
    ...
    def insert(self, i, x):
        ...
        if self.root.publish is not None:
            self.root.publish({"action": ModAction.insert,
                               "path": self._path,
                               "i": i, "x": x})
    ...
    def pop(self, i=-1):
        ...
        if self.root.publish is not None:
            self.root.publish({"action": ModAction.pop,
                               "path": self._path,
                               "i": i})
        return r
    ...
    def __setitem__(self, key, value):
        ...
        if self.root.publish is not None:
            self.root.publish({"action": ModAction.setitem,
                               "path": self._path,
                               "key": key,
                               "value": value})
    ...
    def __delitem__(self, key):
        ...
        if self.root.publish is not None:
            self.root.publish({"action": ModAction.delitem,
                               "path": self._path,
                               "key": key})
...
class Publisher(AsyncioServer):
    ...
    async def _handle_connection_cr(self, reader, writer):
        try:
            ...
            obj = {"action": ModAction.init, "struct": notifier.raw_view}
